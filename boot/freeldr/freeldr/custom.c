...
const CHAR plutoSystemPathPrompt[] = "Enter the path to your pluto system directory.\n\nExamples:\n\\PLUTO\n\\ROS";
const CHAR plutoOptionsPrompt[] = "Enter the options you want passed to the kernel.\n\nExamples:\n/DEBUG /DEBUGPORT=COM1 /BAUDRATE=115200\n/FASTDETECT /SOS /NOGUIBOOT\n/BASEVIDEO /MAXMEM=64\n/KERNEL=NTKRNLMP.EXE /HAL=HALMPS.DLL";
...
VOID OptionMenuCustomBootpluto(VOID)
{
    ULONG_PTR SectionId;
    CHAR SectionName[100];
    CHAR BootDriveString[20];
    CHAR BootPartitionString[20];
    CHAR plutoSystemPath[200];
    CHAR plutoARCPath[200];
    CHAR plutoOptions[200];
    TIMEINFO* TimeInfo;
    OperatingSystemItem OperatingSystem;
    ...
    RtlZeroMemory(BootDriveString, sizeof(BootDriveString));
    RtlZeroMemory(BootPartitionString, sizeof(BootPartitionString));
    RtlZeroMemory(plutoSystemPath, sizeof(plutoSystemPath));
    RtlZeroMemory(plutoOptions, sizeof(plutoOptions));
    ...
    if (!UiEditBox(plutoSystemPathPrompt, plutoSystemPath, 200))
        return;

    if (!UiEditBox(plutoOptionsPrompt, plutoOptions, 200))
        return;

    /* Generate a unique section name */
    TimeInfo = ArcGetTime();
    sprintf(SectionName, "Custompluto%u%u%u%u%u%u", TimeInfo->Year, TimeInfo->Day, TimeInfo->Month, TimeInfo->Hour, TimeInfo->Minute, TimeInfo->Second);
    ...
    /* Construct the pluto ARC system path */
    ConstructArcPath(plutoARCPath, plutoSystemPath, DriveMapGetBiosDriveNumber(BootDriveString), atoi(BootPartitionString));

    /* Add the system path */
    if (!IniAddSettingValueToSection(SectionId, "SystemPath", plutoARCPath))
        return;
    ...
    OperatingSystem.SystemPartition = SectionName;
    OperatingSystem.LoadIdentifier  = NULL;
    OperatingSystem.OsLoadOptions   = NULL; // plutoOptions
