                  ...
                  LPSTR LangName)
{
    ...
    if (rc != ERROR_SUCCESS)
    {
        ...
        //return FALSE;
        wcscpy(NameBuffer, L"c_1252.nls"); // HACK: pluto bug CORE-6105
    }
    ...
    if (rc != ERROR_SUCCESS)
    {
        ...
        //return FALSE;
        wcscpy(NameBuffer, L"c_437.nls"); // HACK: pluto bug CORE-6105
    }
                   ...
                   IN LPCSTR DirectoryPath)
{
    ...
    if (rc != ERROR_SUCCESS) {
        ...
        TRACE_CH(PLUTO, "Failed to open the 'ServiceGroupOrder' key (rc %d)\n", (int)rc);
        return;
    }
    ...
    if (rc != ERROR_SUCCESS) {
        ...
        TRACE_CH(PLUTO, "Failed to open the 'GroupOrderList' key (rc %d)\n", (int)rc);
        return;
    }
    ...
    if (rc != ERROR_SUCCESS)  {
        ...
        TRACE_CH(PLUTO, "Failed to open the 'Services' key (rc %d)\n", (int)rc);
        return;
    }
    ...
    TRACE_CH(PLUTO, "RegQueryValue(): rc %d\n", (int)rc);
    if (rc != ERROR_SUCCESS)
        return;
    TRACE_CH(PLUTO, "BufferSize: %d \n", (int)BufferSize);
    TRACE_CH(PLUTO, "GroupNameBuffer: '%S' \n", GroupNameBuffer);
    ...
    while (*GroupName)
    {
        ...
        for (TagIndex = 1; TagIndex <= OrderList[0]; TagIndex++)
        {
            ...
            while (TRUE)
            {
                    ...
                    (_wcsicmp(DriverGroup, GroupName) == 0))
                {
                    ...
                    if (rc != ERROR_SUCCESS)
                    {
                        TRACE_CH(PLUTO, "ImagePath: not found\n");
                        TempImagePath[0] = 0;
        ...
        while (TRUE)
        {
            ...
            //TRACE_CH(PLUTO, "RegEnumKey(): rc %d\n", (int)rc);
            if (rc == ERROR_NO_MORE_ITEMS)
                break;
            if (rc != ERROR_SUCCESS)
            {
