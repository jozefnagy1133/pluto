...
set PLUTO_SOURCE_DIR=%~dp0

REM Ensure there's no spaces in the source path
echo %PLUTO_SOURCE_DIR%| find " " > NUL
if %ERRORLEVEL% == 0 (
...
set PLUTO_OUTPUT_PATH=output-%BUILD_ENVIRONMENT%-%ARCH%

if "%VS_SOLUTION%" == "1" (
    set PLUTO_OUTPUT_PATH=%PLUTO_OUTPUT_PATH%-sln
)

if "%PLUTO_SOURCE_DIR%" == "%CD%\" (
    set CD_SAME_AS_SOURCE=1
    echo Creating directories in %PLUTO_OUTPUT_PATH%

    if not exist %PLUTO_OUTPUT_PATH% (
        mkdir %PLUTO_OUTPUT_PATH%
    )
...
if "%NEW_STYLE_BUILD%"=="0" (
    ...
    if "%ARCH%" == "arm" (
        ...
        cmake -G %CMAKE_GENERATOR% -DARCH:STRING=%ARCH% "%PLUTO_SOURCE_DIR%"
    )
...
if "%NEW_STYLE_BUILD%"=="0" (
    set BUILD_TOOLS_FLAG=-DPLUTO_BUILD_TOOLS_DIR:PATH="%PLUTO_BUILD_TOOLS_DIR%"
)

if "%BUILD_ENVIRONMENT%" == "MinGW" (
    cmake -G %CMAKE_GENERATOR% -DENABLE_CCACHE:BOOL=0 -DCMAKE_TOOLCHAIN_FILE:FILEPATH=%MINGW_TOOCHAIN_FILE% -DARCH:STRING=%ARCH% %BUILD_TOOLS_FLAG% %* "%PLUTO_SOURCE_DIR%"
) else if %USE_CLANG_CL% == 1 (
        cmake -G %CMAKE_GENERATOR% -DCMAKE_TOOLCHAIN_FILE:FILEPATH=toolchain-msvc.cmake -DARCH:STRING=%ARCH% %BUILD_TOOLS_FLAG% -DUSE_CLANG_CL:BOOL=1 -DRUNTIME_CHECKS:BOOL=%VS_RUNTIME_CHECKS% %* "%PLUTO_SOURCE_DIR%"
) else (
    cmake -G %CMAKE_GENERATOR% -DCMAKE_TOOLCHAIN_FILE:FILEPATH=toolchain-msvc.cmake -DARCH:STRING=%ARCH% %BUILD_TOOLS_FLAG% -DRUNTIME_CHECKS:BOOL=%VS_RUNTIME_CHECKS% %* "%PLUTO_SOURCE_DIR%"
)
...
if "%CD_SAME_AS_SOURCE%" == "1" (
    set ENDV= from %PLUTO_OUTPUT_PATH%
)

if "%VS_SOLUTION%" == "1" (
    set ENDV= You can now use msbuild or open PLUTO.sln%ENDV%.
) else (
